...
export default function SubmitDeathReport() {
  ...
  const submit = async (e) => {
    ...
    setLoading(true);
    try {
      await reportsService.createDeath({
        disease_name: form.disease,
        area_id: form.area,
        death_count: parseInt(form.count, 10),
        notes: form.notes,
        lat: form.lat,
        lng: form.lng,
    ...
    <PageTransition>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <Panel>
            <div className="flex items-start gap-3 p-4 rounded-xl bg-red-50 border border-red-100 mb-6">
              ...
              <div className="text-[12.5px] text-red-800 leading-relaxed">
                Death reports are reviewed promptly and may trigger automated alerts.
                Please double-check details before submission.
              </div>
            </div>
            ...
            <form onSubmit={submit} className="space-y-6">
              ...
              <Field label="Specific location" hint="Optional — pin the precise location.">
                <LocationSearch
              ...
              <Field label="Notes" hint="Optional notes on circumstances, comorbidities, etc.">
                <Textarea
                  rows={6}
        ...
        <Panel>
          ...
          <h3 className="text-[16px] font-semibold tracking-tight text-ink">After submission</h3>
          <ol className="mt-5 space-y-4 text-[13px] text-ink-2">
            {[
              'Submission is recorded against your clinic profile.',
              'Authority dashboards update in real-time.',
